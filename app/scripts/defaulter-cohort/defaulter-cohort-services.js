...
dc.factory('DefaulterCohortService', ['$http', 'spinnerService', 'localStorage.utils', 'DefaulterCohort','EncounterService','PatientService',
  function ($http, spinner, local, DefaulterCohort,EncounterService,PatientService) {
    ...
    DefaulterCohortService.get = function (uuid, callback) {
      ...
      local.get('defaulter-cohort', uuid,true,function(dc) {
        if (dc) {
          callback(dc);
          ...
          DefaulterCohort.get({defaulter_cohort_uuid: uuid}, function (result) {
            ...
            local.set('defaulter-cohort', uuid, result.defaulter_cohort, true);
            var memberUuids = [];
        ...
        PatientService.get(memberUuid, function (patient) {
          if(!patient.error) {
            EncounterService.patientQuery(params, function (encounters) {
              ...
              local.set('openmrs.patient', patient.uuid, patient, true)
            });
          }
    ...
    DefaulterCohortService.getDefaulterCohorts = function (callback) {
      local.getAll("defaulter-cohorts",false,function(dcs) {
      ...
      if (navigator.onLine) {
        ...
        $http.get(url).success(function (retiredPatients) {
          ...
          if (retiredPatients.indexOf("*") != -1) {
            ...
            local.get('defaulter-cohort', uuid, true, function (cohort) {
              ...
              local.set('defaulter-cohort', uuid, cohort, true,callback);
            });
          }
        });
      }
    ...
    DefaulterCohortService.getNew = function (uuid, callback) {
      ...
      $http.get(url).success(function (data) {
        ...
        local.set("defaulter-cohort", data.defaulter_cohort.uuid, data.defaulter_cohort, true);
